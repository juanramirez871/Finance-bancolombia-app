    ...
    return new Promise((resolve, reject) => {
      ...
      const handleChunk = (chunk: string) => {
        ...
        for (const block of blocks) {
          ...
          if (eventName === "error") {
            completed = true;
            reject(new Error(String(payload.message ?? "Sync failed")));
          }
        }
      };

      xhr.open("POST", `${API_BASE_URL}/api/email/sync/stream`);
      xhr.setRequestHeader("Accept", "text/event-stream");
      xhr.setRequestHeader("Content-Type", "application/json");
      if (token) {
        xhr.setRequestHeader("Authorization", `Bearer ${token}`);
      ...
      xhr.onerror = () => {
        if (!completed) {
          reject(new Error("Network error during sync"));
        }
      };
      ...
      xhr.onload = () => {
        ...
        if (!completed) {
          reject(new Error(`Sync failed (${xhr.status})`));
        }
      };
